...
def main(path):
    ...
    for object in data['dataset']['samples']:
        ...
        if object["labels"].keys() != label_keys and set(object["labels"]) != set(["image_attributes"]):
            print(f"    {object['name']} labels does not have the expected keys.")

        if object["labels"]["image_attributes"].keys() != img_attributes_keys:
            print(f"    {object['name']} image_attributes does not have the expected keys.")
    ...
    for object in data['dataset']['samples']:
        ...
        for label_i in labels:
            if not has_no_spelling_mistakes(label_i):
                print(f"    {object['name']}: detected spelling error in label '{label_i}'")
            if "  " in label_i:
                print(f"    {object['name']}: detected multiple consecutive spaces in label '{label_i}'")
