...
def set_based_ranking(
    ...
    for gt_id, distance, index in tqdm.tqdm(
        ...
        (
            index_synonym_scores,
            index_vis_sim_scores,
            index_depiction_scores,
            _,
        ) = ([], [], [], [])
        if consider_synonyms:
            for pred_rank in syn_ranks:
                left_box_constr = 1 - min(
                    (0, (pred_rank - syn_rank_l) / (L - syn_rank_l))
                )
                right_box_constr = 1 - max(
                    (0, (pred_rank - syn_rank_r) / (L - syn_rank_r))
                )
        ...
        if consider_depiction:
            for pred_rank in depiction_ranks:
                left_box_constr = 1 - min(
                    (0, (pred_rank - depiction_rank_l) / (L - depiction_rank_l))
                )
                right_box_constr = 1 - max(
                    (0, (pred_rank - depiction_rank_r) / (L - depiction_rank_r))
                )
        ...
        if consider_vis_sim:
            for pred_rank in vis_sim_ranks:
                left_box_constr = 1 - min(
                    (0, (pred_rank - vis_sim_rank_l) / (L - vis_sim_rank_l))
                )
                right_box_constr = 1 - max(
                    (0, (pred_rank - vis_sim_rank_r) / (L - vis_sim_rank_r))
                )
